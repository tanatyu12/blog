...
class GitGateway {
  ...
  getMedia() {
    return Promise.all([this.backend.getMedia(), this.getLargeMediaClient()]).then(async (_ref2) => {
      ...
      return mediaFiles.map((_ref4) => {
        let id = _ref4.id,
            displayURL = _ref4.displayURL,
            path = _ref4.path,
            rest = _objectWithoutProperties(_ref4, ["id", "displayURL", "path"]);
  ...
  _getLargeMediaClient() {
    const netlifyLargeMediaEnabledPromise = this.api.readFile('.lfsconfig').then(_ini.default.decode).then((_ref5) => {
      let url = _ref5.lfs.url;
      return new URL(url);
    }).then(lfsURL => ({
    ...
    return Promise.all([netlifyLargeMediaEnabledPromise, lfsPatternsPromise]).then((_ref6) => {
      let _ref7 = _slicedToArray(_ref6, 2),
          maybeEnabled = _ref7[0].enabled,
          _ref7$ = _ref7[1],
          patterns = _ref7$.patterns,
          patternsErr = _ref7$.err;
  ...
  async getLargeMediaDisplayURLs(mediaFiles) {
    ...
    const largeMediaItems = mediaFiles.filter((_ref8) => {
      let path = _ref8.path;
    ...
    return this.backend.fetchFiles(largeMediaItems).then(items => items.map((_ref10) => {
      let sha = _ref10.file.sha,
          data = _ref10.data;
    ...
    })).then(_unzip2.default).then((_ref11) => {
      ...
      let _ref14 = _slicedToArray(_ref13, 2),
          idMaps = _ref14[0],
          resourceMap = _ref14[1];

      return idMaps.map((_ref15) => {
        let pointerId = _ref15.pointerId,
            resourceId = _ref15.resourceId;
