...
const USE_FIELD_PREFIX = 'fields.'; // Allow `fields.` prefix in placeholder to override built in replacements
...
function compileSlug(template, date) {
...
function slugFormatter(collection, entryData, slugConfig) {
  ...
  const processSlug = (0, _flow2.default)([compileSlug, prepareSlug, (0, _partialRight2.default)(_urlHelper.sanitizeSlug, slugConfig)]);
...
function parsePreviewPathDate(collection, entry) {
  const dateField = collection.get('preview_path_date_field') || (0, _collections.selectInferedField)(collection, 'date');
...
function createPreviewUrl(baseUrl, collection, slug, slugConfig, entry) {
  ...
  const date = parsePreviewPathDate(collection, entry); // Prepare and sanitize slug variables only, leave the rest of the
...
class Backend {
  ...
  async listAllEntries(collection) {
    ...
    while (cursor && cursor.actions.includes('next')) {
      const _ref3 = await this.traverseCursor(cursor, 'next'),
            newEntries = _ref3.entries,
  ...
  async search(collections, searchTerm) {
    ...
    const collectionEntriesRequests = collections.map(async collection => {
      ...
      const searchFields = [(0, _collections.selectInferedField)(collection, 'title'), (0, _collections.selectInferedField)(collection, 'shortTitle'), (0, _collections.selectInferedField)(collection, 'author')];
    ...
    const hits = entries.filter((_ref4) => {
      let score = _ref4.score;
      return score > 5;
  ...
  traverseCursor(cursor, action) {
    ...
    return this.implementation.traverseCursor(unwrappedCursor, action).then(async (_ref5) => {
      let entries = _ref5.entries,
          newCursor = _ref5.cursor;
      return {
  ...
  async getDeployPreview(collection, slug, entry) {
    let _ref6 = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {},
        _ref6$maxAttempts = _ref6.maxAttempts,
        maxAttempts = _ref6$maxAttempts === void 0 ? 1 : _ref6$maxAttempts,
        _ref6$interval = _ref6.interval,
        interval = _ref6$interval === void 0 ? 5000 : _ref6$interval;
  ...
  persistEntry(config, collection, entryDraft, MediaFiles, integrations) {
